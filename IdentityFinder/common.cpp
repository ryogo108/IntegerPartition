...
vector<long long>countSuitablePartitions(int maxSizeOfPartition,vector<part> & ps,function<bool(Par &)>isSuitable){
  //条件fを満たす分割の数を数える
  ...
  for(int sizeOfPartition=0; sizeOfPartition <= maxSizeOfPartition; sizeOfPartition++){
    ...
    numOfSuitablePartitionsBySize.push_back(cnt);
  }
